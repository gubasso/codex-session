...
| 03    | `03-remaining-skills-dctl.md` | Remaining skills + dctl config       | done   | 2026-05-28 |
...
- **`prex-resume` is not a separate skill (cross-repo audit finding, C6).** This plan's round 02-03 text (e.g. `02-prex-sandbox-fix.md` Step 7, `03-remaining-skills-dctl.md`, `01-docs-and-conventions.md`) repeatedly frames resume as a companion `~/.claude/skills/prex-resume/SKILL.md` alongside `prex/SKILL.md`. The `done-plans-fidelity-cleanup` round 02 read-only audit confirmed no separate `prex-resume` skill exists in the live `~/.claude/skills/` tree (`find ~/.claude/skills -iname '*resume*'` → no matches): resume is unified inside the single `prex/SKILL.md` (stage-1 exec, stage-3 `exec resume`, and the resume fallback). The sandbox-fix substance still landed — the unified `--dangerously-bypass-approvals-and-sandbox` pattern and XDG run dirs are present in `prex/SKILL.md` — only the two-skill topology in the historical round text is inaccurate. The original round text is left intact as the historical record; evidence is in `.plan/done-plans-fidelity-cleanup/cross-repo-verification-report.md`.
