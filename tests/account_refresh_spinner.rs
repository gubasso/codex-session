...
fn run_refresh_via_pty(env: &TestEnv, child_bin: &Path, account: &str) -> (ExitStatus, String) {
    let bin = assert_cmd::cargo::cargo_bin("codex-session");
    let transcript = env.tmp.path().join(format!("refresh-{account}.typescript"));
...
#[test]
fn refresh_noninteractive_rejects_without_spinner_artifacts() {
    let env = TestEnv::new();
    env.seed_account("target", r#"{"tokens":{"access_token":"old"}}"#);

    let output = env
        .cmd()
        .args(["account", "refresh", "target"])
        .assert()
        .failure()
        .get_output()
        .clone();
...
#[test]
fn refresh_success_persists_credentials_via_pty() {
    skip_without_script!();
    let env = TestEnv::new();
    env.seed_account("target", r#"{"tokens":{"access_token":"old"}}"#);
    let child_dir = env.make_fake_codex_in_dir("refresh-child", &fake_login_script());
    let child_bin = child_dir.join("codex");

    let (status, transcript) = run_refresh_via_pty(&env, &child_bin, "target");
    assert!(
        status.success(),
        "refresh failed: {status}; transcript:\n{transcript}"
    );

    let seed = std::fs::read_to_string(env.named_account_auth_seed("target")).unwrap();
    assert!(seed.contains("fresh-at"), "seed: {seed}");
    assert!(seed.contains("fresh-rt"), "seed: {seed}");
}

#[test]
fn refresh_pty_transcript_has_clean_child_login_line() {
    skip_without_script!();
    let env = TestEnv::new();
    env.seed_account("target", r#"{"tokens":{"access_token":"old"}}"#);
    let child_dir = env.make_fake_codex_in_dir("refresh-child-clean", &fake_login_script());
    let child_bin = child_dir.join("codex");
