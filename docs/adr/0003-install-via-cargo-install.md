...
its existing project tooling already documents `cargo install --path . --force`
as the installation path.

## Decision

Installation is owned by Cargo. The supported install flow is `cargo install
--path . --force`, wrapped by `just install`. The bash staging and relink flow
is superseded and is not reimplemented in Rust.
